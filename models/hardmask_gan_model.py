...
class HardMaskGANModel(BaseModel):
    ...
    def get_gaussian_kernel(kernel_size=3, sigma=2, channels=3):
        ...
        gaussian_filter = nn.Sequential(nn.ConstantPad2d(padding=kernel_size // 2, value=-1),
                                        nn.Conv2d(in_channels=channels, out_channels=channels,
                                                  kernel_size=kernel_size, groups=channels, bias=False, )
                                        )
        gaussian_filter[1].weight.data = gaussian_kernel
        gaussian_filter[1].weight.requires_grad = False
    ...
    def initialize(self, opt):
        ...
        self.gaus_filter = self.get_gaussian_kernel(5, 2, 1).to(self.device)
