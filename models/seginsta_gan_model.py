...
class SegInstaGANModel(BaseModel):
	def name(self):
		return 'SegInstaGANModel'
	...
	def initialize(self, opt):
		...
		self.netG_A = networks.define_G(opt.input_nc, opt.output_nc, opt.ngf, 'seg', opt.norm, not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids)
		self.netG_B = networks.define_G(opt.output_nc, opt.input_nc, opt.ngf, 'seg', opt.norm, not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids)
		if self.isTrain:
			use_sigmoid = opt.no_lsgan
			self.netD_A = networks.define_D(opt.output_nc, opt.ndf, 'seg', opt.n_layers_D, opt.norm, use_sigmoid, opt.init_type, opt.init_gain, self.gpu_ids)
			self.netD_B = networks.define_D(opt.input_nc, opt.ndf, 'seg', opt.n_layers_D, opt.norm, use_sigmoid, opt.init_type, opt.init_gain, self.gpu_ids)
	...
	def set_input(self, input):
		...
		self.real_A_segs = real_A_segs.to(self.device)
		self.real_B_segs = real_B_segs.to(self.device)
		self.real_A = torch.cat([self.real_A_img, self.real_A_segs], dim=1)
		self.real_B = torch.cat([self.real_B_img, self.real_B_segs], dim=1)
		self.real_A_seg = self.real_A_segs  # merged mask
		self.real_B_seg = self.real_B_segs  # merged mask
	...
	def optimize_parameters(self):
		...
		for i in range(self.ins_iter):
			...
			if i == self.ins_iter - 1:  # last
				self.fake_A_img = self.fake_A_img_sng
				self.fake_B_img = self.fake_B_img_sng
				self.fake_A_seg = self.fake_A_seg_mul
				self.fake_B_seg = self.fake_B_seg_mul
				self.rec_A_seg = torch.cat(self.rec_A_seg_list, dim=1)
				self.rec_B_seg = torch.cat(self.rec_B_seg_list, dim=1)
